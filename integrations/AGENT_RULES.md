...
4b. Never summarize a long original to fit one call. Only the trusted rewrite worker may segment plain text or a documented structure-aware JSON, HTML, Android-resource XML, conservative Markdown, GNU PO, Apple `.strings`, or SRT/WebVTT profile. A source-bearing container must use a trusted target-only projection for the first review: PO reviewers receive decoded `msgstr` values, Apple reviewers receive decoded localized values, Android XML reviewers receive decoded selected string/quantity values, JSON/ARB reviewers receive decoded non-empty string values, HTML reviewers receive decoded visible text plus approved linguistic attributes, and subtitle reviewers receive ordered cue payloads—never source fields, resource names, keys, paths, comments, markup, scripts, code, link destinations, technical attributes, subtitle headers/IDs/timestamps/settings/metadata, non-string scalars or layout. The preservation reviewer receives the complete assembled result and original; every unsupported long structure remains blocked.
