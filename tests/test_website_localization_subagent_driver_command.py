...
key = request['provider_execution_key']
digest = request['provider_request_sha256']
...
if mode in {'terminal', 'terminal_wrong_binding'}:
    response = {
        'schema': 'translate-native.subagent-review-command-response.v1',
        'operation': operation,
        'protocol_version': '1',
        'driver_id': 'standard-command-driver',
        'driver_version': 'command-v1',
    ...
    if mode == 'terminal_wrong_binding':
        response['provider_execution_key'] = '0' * 64
elif mode == 'nonzero':
    raise SystemExit(23)
elif mode == 'oversize':
...
class CommandDriverTests(unittest.TestCase):
    ...
    def test_finnish_full_path_is_source_blind_and_environment_clean(self):
        ...
        self.assertEqual(captures[0]["environment"], {
            "LANG": "C.UTF-8", "LC_ALL": "C.UTF-8", "TZ": "UTC",
            "BLUN_SUBAGENT_COMMAND_PROTOCOL": "1",
        })
        self.assertEqual(captures[0]["cwd"], str(self.root))
        self.assertNotIn("SYNTHETIC_AMBIENT_SECRET", captures[0]["environment"])
        self.assertGreaterEqual(len(captures[0]["fds"]), 5)
        self.assertLessEqual(len(captures[0]["fds"]), 8)
    ...
    def test_maltese_native_then_fidelity_have_separate_reduced_inputs(self):
        ...
        captures = json.loads(self.capture.read_text(encoding="utf-8"))
        self.assertEqual(
            [item["request"]["assignment"]["phase"] for item in captures],
            ["target_native", "source_fidelity"],
        )
        self.assertNotIn("source", captures[0]["request"]["model_input"]["input"])
        self.assertIn("source", captures[1]["request"]["model_input"]["input"])
    ...
    def test_reserved_unknown_reconcile_reaches_command_without_starting(self):
        ...
        captures = json.loads(self.capture.read_text(encoding="utf-8"))
        self.assertEqual(len(captures), 1)
        self.assertEqual(captures[0]["request"]["operation"], "reconcile")
        self.assertNotIn("model_input", captures[0]["request"])
        self.assertNotIn("budgets", captures[0]["request"])
