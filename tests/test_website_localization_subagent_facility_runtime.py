...
class FacilityRuntimeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.module_path.write_text(
            ...
            "            else:\n"
            "                response = {'schema': 'blun.website-localization-review.v3', 'phase': model_input['phase'], 'locale': locale, 'status': 'PASS', 'confidence': 'high', 'blocking_defects': [], 'major_defects': [], 'uncertainties': []}\n"
            "                if model_input['phase'] == 'target_native': response['holistic_assessment'] = {'reads_as_native_original': True, 'reason': 'Synthetic whole-text facility fixture.', 'repair_scope': 'none', 'dimensions': {name: 'PASS' for name in ('idiom_and_word_choice','syntax_and_information_flow','rhythm_and_cohesion','register_tone_and_audience','voice_genre_and_intentional_repetition')}}\n"
