...
| A nominally source-blind Apple `.strings` review exposes keys or comments from the original container | The first reviewer has no `source` field but its candidate still contains stable identifiers, source-language comments, URLs or empty host-owned entries | Parse the exact assembled catalog in trusted host code and send only ordered decoded non-empty localized values; reserve the complete catalog and original for preservation review | Short and long Finnish fixtures place unique sentinels in keys and comments; Finnish, Maltese and Arabic adapter tests prove only localized values reach native review while fidelity retains exact containers |
| A nominally source-blind Android XML review exposes resource names, comments or attributes | The first reviewer has no `source` field but its candidate is still the complete assembled XML container | Parse the exact assembled Android resources in trusted host code and send only ordered decoded selected string, plural and string-array values; reserve the complete XML and original for preservation review | Short and long fixtures place unique sentinels in names, comments and opaque resources; Finnish, Maltese and Arabic adapter tests prove only localized values reach native review while fidelity retains exact XML |
| A nominally source-blind JSON/ARB review exposes keys, paths or technical scalars | The first reviewer has no `source` field but its candidate is still the complete assembled JSON container | Parse the exact assembled JSON in trusted host code and send only ordered decoded non-empty string values; reserve keys, paths, structure, scalars and the complete container for preservation review | Short and long fixtures place unique sentinels in keys and booleans; Finnish, Maltese and Arabic adapter tests prove only string values reach native review while fidelity retains exact JSON |
| A nominally source-blind HTML review exposes source-bearing markup or executable context | The first reviewer has no `source` field but its candidate still contains comments, scripts, link destinations, IDs or technical attributes | Parse the exact assembled HTML in trusted host code and send only ordered decoded visible text plus approved linguistic attributes; reserve the complete container and original for preservation review | Short and long fixtures place unique sentinels in comments, IDs, scripts, code and links; Finnish, Maltese and Arabic adapter tests prove only linguistic values reach native review while fidelity retains exact HTML |
| A nominally source-blind subtitle review exposes timing or source-bearing container metadata | The first reviewer has no `source` field but its candidate still contains WebVTT headers, cue IDs, timestamps, settings or metadata blocks | Parse the exact assembled SRT/WebVTT in trusted host code and send only ordered complete cue payloads; reserve the full subtitle and original for preservation review | Short SRT and long Finnish, Maltese and Arabic fixtures prove only cue copy reaches native review; kind, hash, scope and full-target mutations block in the Guard |
...
| Whole-document review is silently replaced by per-span approval | Per-span output looks fluent although cross-element rhythm, terminology, or meaning drifts | Reassemble first, then run one source-blind native review of the complete ordered target-value projection followed by a separate full-XML original-preservation review; only the Guard releases | Finnish, Maltese and Arabic synthetic documents show metadata isolation and ordered complete-document review; uncertainty remains blocked |
...
| XLIFF support leaks source segments into the nominally source-blind review | A complete XLIFF container or its `source`, unit IDs, notes or state metadata appears in the first reviewer input | Recognize only exact XLIFF 1.2/2.0 namespace-version pairs and project only ordered decoded simple `target` values; reserve the complete container for fidelity review | Short and long sentinel fixtures prove sources, IDs, notes and attributes are absent from native review while fidelity retains the byte-exact XLIFF |
| XLIFF inline codes or version confusion change ownership | An unknown namespace/version or a `target` containing inline elements is accepted as plain prose | Select only 1.2 `trans-unit/target` and 2.0 `segment/target` plain text; reject inline target content, version/namespace mismatch, missing targets and `translate=no`-only documents before creator access | Planner and worker adversarial tests make zero creator calls; Finnish, Maltese and Arabic fixtures cross adapter and Guard under the exact selector profile |
