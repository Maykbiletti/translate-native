...
`integrations/website_localization_subagent_host.py`. The WSGI endpoint
authenticates before reading JSON, resolves only pinned locale/profile routes,
assigns reviewer identity and budgets from trusted host policy, and sends a
source-blind reviewer only the target candidate and permitted target-language
profile. Source-fidelity review is accepted only after the exact successful
native receipt for the same candidate, target policy, authenticated principal,
host, provider and model generation.

The host commits the complete HMAC-attested response to an atomic SQLite
journal before returning HTTP 200. Exact retries replay identical bytes;
changed requests conflict. Crash continuation requires a launcher with atomic
`execute_idempotent` physical-start deduplication plus `reconcile`, so an
expired database lease cannot start the same model job twice. Invalid reviewer
identity, execution metadata or complete phase-specific response structure is
never attested.

Synthetic Finnish and Maltese tests exercise ordinary target-language answers
and ordered website translation reviews through the real HTTPS client and WSGI
endpoint, including source isolation, self-review, replay, response loss,
concurrency and expired-worker races. They prove protocol behavior only. A
deployment must still supply a real model-specific isolated launcher and real
qualified native-language evidence; these tests make no DeepL-quality claim.
...
Version 6.187.0 supplies the provider-neutral durable host endpoint and its
strict launcher contract. A deployment must still supply a real isolated
model-specific launcher, and qualified native evidence or independent-model
...
Versions 6.186.0 and 6.187.0 supply the protected generic HTTPS factory and
durable provider-neutral host endpoint. Without configured trust files, a real
model-specific launcher and actual reviewer execution, readiness and response
release deliberately stay blocked.

### Version 6.184.0: authenticated host-subagent HTTPS bridge

The website review adapter can now connect to a provider-neutral trusted host
through a bounded, authenticated [HTTPS contract](docs/host-subagent-reviews.md).
Every request is bound to the exact isolated task and host-only control,
deduplicated by the execution key, and accepted only with a locally verified
host attestation over the complete response and receipt. Native requests are
...
The website worker now accepts a [host-subagent review adapter](docs/host-subagent-reviews.md).
The creator cannot serve as its own reviewer: native and fidelity checks run
through distinct, host-verified identities and isolated contexts. Native review
excludes source-bearing project metadata. Verified execution receipts are bound
...
Version 6.182.0 exposes the hosted external watcher-recovery runtime through a
strict provider-neutral HTTPS/WSGI boundary. A separately scoped operator can
commit one explicit idempotent recovery operation, while independent status
and readiness routes return only content-free state, phase, attempt, and stable
error information. The exact body is authenticated before JSON parsing, and a
changed operation cannot reuse the durable recovery namespace.

An independently scoped, state-free OpenAPI 3.1 route publishes the canonical
origin-free contract and its deterministic digests. All four routes reject
unsafe transport framing, queries, wrong scopes, malformed principals, forged
runtime projections, and storage or worker failures fail-closed. Operation
identities, endpoints, credentials, benchmark content, locales, providers,
models, and remote prose never enter a response.

### Version 6.181.0: hosted external watcher recovery

Version 6.181.0 adds the production composition root for the external
benchmark-watcher recovery runner. It validates the complete client and retry
policy before creating storage, owns one process-bound SQLite connection,
requires an owner-only identity-stable database file, and serializes every
operation around file guards.

An explicitly named recovery can now run in one supervised non-daemon worker.
Startup never invents recovery intent, shutdown signals and joins before
closing SQLite, and a blocked network request leaves storage open after the
bounded join timeout. Content-free readiness exposes only worker state,
recovery phase, attempts, and stable error codes; operation names, benchmark
content, locales, providers, models, credentials, and remote prose remain
absent.

### Version 6.180.0: crash-resumable external watcher recovery

Version 6.180.0 adds an explicitly started, durable operator runner around the
provider-neutral benchmark-watcher recovery client. It performs OpenAPI
verification, failed-generation observation, and idempotent rearm as three
separately leased network steps. The failed generation and exact derived
request identity are committed before the rearm request, so a process crash or
lost response can resume without inventing a different idempotency key.

The SQLite state is pinned to the HTTPS origin and retry policy, uses bounded
exponential backoff, recovers only confirmed expired leases, and stores only
content-free hashes, stable error codes, counters, and timestamps. Parallel
operators cannot issue the same step concurrently; terminal contract,
authentication, parser, state, and integrity failures remain fail-closed. A
watcher that is no longer failed finishes as `not_required` without sending a
rearm request.

### Version 6.179.0: contract-bound watcher recovery discovery

Version 6.179.0 gives external operator services an authenticated, origin-free
OpenAPI 3.1 description of the benchmark-watcher status and recovery boundary.
The strictly bodyless `/v1/benchmarks/watcher/openapi` route has its own read
scope and builds the document entirely from a closed versioned contract; it
never reads watcher state or exposes campaign, locale, provider, model, or
benchmark data.

Request and response limits are now separate, so the larger contract response
does not increase the accepted recovery-request size. The provider-neutral
client reconstructs the complete expected document locally and requires the
contract digest, document digest, and exact value to agree. Stale, altered, or
self-rehashed substitutions therefore fail closed before an operator can
generate or trust a recovery integration.

### Version 6.178.0: observable watcher recovery

Version 6.178.0 completes the external benchmark-watcher recovery path with a
separately authenticated, strictly bodyless
`GET /v1/benchmarks/watcher/status` operation. It returns only the current
state and, when the watcher is terminally failed, the exact content-free
attempt count, failure time, and stable error code required by the existing
idempotent rearm operation. Campaign, policy, suite, locale, provider, model,
credentials, and benchmark content remain undisclosed.

The provider-neutral client can now read that closed status and perform the
complete status-to-rearm flow while preserving a caller-supplied request ID.
Read and write scopes remain distinct, authentication completes before state
access, body-bearing status requests fail before authentication, and the
write-side generation comparison remains authoritative if state changes
between the two requests.

### Version 6.177.0: provider-neutral watcher recovery client

Version 6.177.0 makes the authenticated benchmark-watcher recovery boundary
usable without provider-specific HTTP code. The strict client accepts one
operator-supplied credential adapter, pins an HTTPS origin, builds one
canonical failed-generation request, and binds the credential context,
request-body hash, and `Idempotency-Key` to those exact bytes.

An uncertain network result is explicitly retryable with the same request ID
and body. Redirects, unsafe origins, credential attempts to override protected
headers, ambiguous response framing, duplicate JSON keys, oversized payloads,
and receipts for another attempt, failure time, or error code fail closed. A
valid server conflict remains terminal; only verified retryable service or
transport failures invite a replay.
...
runtime's outer supervisor lease. The watcher first claims its own durable,
token-bound lease, then invokes the host operation guard immediately before
the strict client's network access. The central runtime supplies its real
`renew_active_lease` guard with the exact watcher lease duration.

If that renewal fails, the remote client is never called and no stale process
may complete the attempt. The inner lease remains intact until its bounded
expiry, so another supervisor can recover it through the existing crash path.
Invalid guards are rejected before state changes, while live leases and future
retry deadlines still perform neither a guard call nor a network read.

### Version 6.174.0: benchmark watcher in the production runtime

Version 6.174.0 closes the deployment gap between the durable benchmark
watcher and the provider-neutral localization service. The central composition
root now accepts one exact `benchmark_watch` configuration, validates its
client binding, worker identity, retry policy, distinct SQLite connection, and
lease hierarchy before writing service state, and constructs the watcher as an
owned runtime component.

Each supervised tick still prioritizes customer translation, approval, and
publication work, then local benchmark execution, and only then one due report
watch attempt. A verified `PASS` becomes a successful benchmark tick; a valid
`BLOCK`, terminal failure, malformed state, or lost lease remains explicitly
blocked. Retryable failures retain the watcher's durable backoff. The same
instance is wired into authenticated service health, so execution and
observation cannot silently refer to different databases or policies.
...
crash-resumable operator workflow. One campaign-, policy-, and suite-bound
SQLite generation now claims report reads with random expiring leases, recovers
abandoned work after expiry, applies capped exponential backoff, and stops at a
fixed attempt ceiling.

Successful retrieval persists only the verified report digest, aggregate
`PASS` or `BLOCK` decision, stable block reasons, locale count, and completion
time. It never stores benchmark prose or evidence. A valid `BLOCK` report is a
successful final retrieval but keeps watcher health blocked and never grants a
superiority claim. Terminal failures require explicit operator rearm, while a
verified final result cannot be replaced or fetched twice.

### Version 6.171.0: contract-bound benchmark OpenAPI

Version 6.171.0 gives external dashboards, CMS adapters, and website backends
an authenticated, origin-free OpenAPI 3.1 description of the benchmark status,
report, and discovery routes. The document fixes the route methods, response
schemas, error statuses, authentication contracts, report schema, state values,
and transport limit without exposing deployment or campaign data.

The reference client first verifies its pinned campaign, policy, suite, and
validity window. It then reconstructs the entire expected document locally and
requires the contract digest, document digest, and complete value all to match.
A stale or self-rehashed substitute therefore fails closed before a CMS can
generate or trust a client from it.

### Version 6.170.0: fail-closed benchmark report client

Version 6.170.0 adds a provider-neutral HTTPS client for dashboards, CMS
adapters, and website backends that consume stored benchmark evidence. The
client pins the exact origin, campaign, policy, and suite; checks content-free
campaign status before report access; verifies strict response framing and the
canonical report digest; and rechecks expiry after both bounded requests.

Incomplete, blocked, expired, foreign, redirected, malformed, or digest-stale
evidence fails closed with stable content-free reasons. A valid report whose
decision is `BLOCK` remains a successful diagnostic result with its existing
claim-block reasons intact; transport success alone never permits a
superiority claim.

### Version 6.169.0: axis-visible benchmark defects

Version 6.169.0 derives separate candidate and baseline Major and Blocking
finding totals for each independent benchmark axis. Each axis also reports how
many cases contain those defects, using only the exact phase-bound hashes from
the already validated signed case evidence.

A candidate defect now blocks the affected `target_native` or
`source_fidelity` axis with a stable severity reason even when its win rate and
sign test would otherwise pass. Baseline defects remain visible for comparison
without authorizing the candidate. Reports store only counts and fixed reason
codes; no finding text, target content, or reviewer prose is added.

### Version 6.168.0: phase-bound benchmark findings

Version 6.168.0 preserves separate, content-free finding registries for the
source-blind native review and the later source-fidelity review. Each signed
pass records the exact candidate and baseline hashes by severity; aggregate
defect counts are accepted only when they can be reconstructed from both
ordered phases.

Commercial offer verdicts must now reference the exact source-fidelity
registry rather than any finding from the case. Swapped phases, omitted or
duplicated hashes, stale totals, and target-native findings relabelled as
commercial fidelity all fail closed. Reviewer excerpts and reasons remain
absent from signed evidence and reports.

### Version 6.167.0: finding-bound commercial offer verdicts

Version 6.167.0 retains the exact content-free finding hash behind every
commercial Major or Blocking offer verdict. The reviewer still returns a
zero-based defect reference, but the validated result resolves that reference
before unblinding and stores each offer as one ordered record containing its
index, status, and finding hash.

Signed candidate and baseline evidence now also carries the corresponding
source-fidelity finding registries by severity. Report validation rejects
missing, invented, foreign, wrong-severity, or unexpected hashes; equivalent
and absent offers require `null`. Reviewer excerpts and reasons are discarded,
and the target-native pass remains unchanged and source-blind.

### Version 6.166.0: decision-bound commercial benchmark evidence

Version 6.166.0 binds every commercial dimension decision to the exact
validated target-offer registry for its anonymous variant. A decision with a
missing, stale, foreign, or swapped registry digest now fails before benchmark
acceptance, including across the provider-neutral HTTPS adapter and durable
review store.

Unblinding preserves the exact candidate or baseline registry digest on every
signed dimension row and report validation compares each row with its
corresponding top-level digest. A valid aggregate or a separately valid
registry can therefore no longer authorize a decision about another target
partition. The target-native pass remains unchanged and receives no commercial
registry or decision binding.

### Version 6.165.0: target-bound commercial benchmark offers

Version 6.165.0 requires the source-fidelity reviewer to map every opaque
offer index onto the exact text of each anonymous target variant. Each mapping
is a complete semantic partition in Unicode code points, binds the target
length and UTF-8 SHA-256 digest, distinguishes offer-owned from shared target
spans, and carries its own canonical digest. The reviewer—not a regular
expression—must identify semantically equivalent, reordered, inflected,
spelled-out, or locale-formatted commercial claims.

Gaps, overlaps, empty offers, wrong indexes, byte-offset assumptions, swapped
variants, stale target hashes, and self-rehashed substitutions block at the
runtime, HTTPS, and durable-review boundaries. After unblinding, only the
candidate and baseline registry digests enter signed case evidence. The
target-native pass remains free of source and commercial registry data.

### Version 6.164.0: source-bound commercial benchmark offers

Version 6.164.0 binds every opaque benchmark offer index to a manually
versioned partition of the exact commercial source. Each registry uses ordered
Unicode-code-point spans, distinguishes offer-owned text from explicitly
shared conditions, covers the source without gaps or overlaps, and carries a
canonical SHA-256 digest.

Only the source-fidelity reviewer receives the registry. The HTTPS boundary,
durable review store and signed case result validate its exact source length,
partition and digest; the target-native pass remains source-blind. This makes
per-offer price and condition verdicts reproducible without regex extraction,
product names, live prices or provider-specific assumptions.

### Version 6.163.0: offer-complete commercial benchmark verdicts

Version 6.163.0 makes every commercial benchmark decision complete across all
offers in the source fixture. The versioned suite registers an opaque offer
count for each of its eight commercial cases. Source-fidelity review must now
return one ordered status per offer, dimension and anonymous variant; missing,
duplicated or reordered indexes fail closed.

Each dimension aggregate is derived by fixed severity from its offer matrix, so
a harmless tier cannot hide another tier's major or blocking defect. Signed
case evidence retains the full text-free matrix, and locale/content reports
publish both case-level and offer-level status counts. Native review remains a
separate source-blind pass, and no provider, product catalogue, brand or live
price is built into the contract.

### Version 6.162.0: managed operator polling contract

Version 6.162.0 completes the durable poller's host-facing runtime path. A
synchronous `run_forever` loop now performs due work, waits interruptibly until
the next lease or retry boundary, wakes at a bounded interval for external
operator rearm, and exits without touching durable state when already stopped.

The separate read-only poller-health contract distinguishes healthy,
degraded, and blocked operation; reports due work, active or expired leases,
attempt ceilings and the last stable error; and includes only the prior
report's canonical digest, aggregate status, stable reason codes and counts.
It contains no tenant, locale, provider or website-content identifiers.

### Version 6.161.0: durable operator health polling

Version 6.161.0 adds a crash-resumable SQLite scheduler around the strict
operator health client. Multiple operator processes now share an atomic lease,
an abandoned poll becomes eligible only after its lease expires, and each
attempt still performs at most one authenticated network request.

Retryable failures use capped exponential backoff and stop at an explicit
ceiling; terminal failures require an explicit operator rearm. A valid blocked
assessment is preserved as a successful observation. Durable state contains
only the canonical report digest, aggregate status, stable reason codes and
cardinalities—never report payloads, site/version IDs, locale names, provider
identities, credentials or website content.

### Version 6.160.0: provider-neutral operator health client

Version 6.160.0 adds a bounded HTTPS client for the authenticated, read-only
service health endpoint. Operators can now consume the full monitor contract
without writing custom response parsing or collapsing a valid blocked report
into a generic network failure.

The client preserves policy outage, verified drift, and integrity reasons from
valid `503` reports; independently rejects stale or future-dated assessments,
duplicate JSON keys, status/schema mismatches, missing security headers,
redirects, unsafe origins, oversized responses, and ambiguous authentication
headers; and exposes only stable content-free failures with explicit retry
decisions. Authentication remains host-configured and no provider is fixed.

### Version 6.159.0: fail-closed policy-aware service health

Version 6.159.0 revalidates every approved locale against its current quality
policy during the general read-only health check. A temporary resolver outage
now blocks service health with the stable, content-free
`release.policy_unavailable` reason instead of leaving an unpublishable website
version under a healthy release component.

Verified policy drift reports `release.policy_stale`; malformed or otherwise
unexpected release evidence reports `release.integrity_failed`. A proven stale
or invalid locale takes precedence over a simultaneous resolver outage, while
the per-locale evidence codes remain visible for diagnosis. The health pass
does not mutate queue, release, or CMS state and never exposes resolver detail.

### Version 6.158.0: retryable lifecycle-policy resolution

Version 6.158.0 carries the publication-policy distinction across the public,
authenticated lifecycle path. A temporary target-locale policy resolver outage
now returns the content-free `cms.release.policy_unavailable` error as a
retryable `503`; the durable source-CMS lifecycle monitor applies its existing
bounded backoff and revalidates the policy on recovery.

Policy drift, malformed release evidence, invalid signatures, and mixed
outage-plus-integrity failures retain precedence as the terminal
`cms.release.integrity_failed` conflict. Lifecycle reads remain read-only and
cannot call the publisher, while the HTTP client no longer retries proven
release-integrity failures.

### Version 6.157.0: retryable publication-policy resolution

Version 6.157.0 distinguishes an authenticated release whose locale policy is
provably stale from a temporary failure to resolve the current policy. Stale
evidence remains terminal and never reaches the publisher. Resolver outages
remain fail-closed but now consume bounded delivery attempts with the existing
exponential backoff instead of being misclassified and quarantined as drift.

The dispatcher can defer an unavailable entry and continue to a valid following
delivery in the same bounded transaction. It repeats the distinction after the
outer operation guard immediately before network access, exposes only the stable
`policy_unavailable` reason in delivery state and health, and never persists a
private resolver diagnostic or calls the publisher while policy is unknown.

### Version 6.156.0: fail-closed publication dispatch policy

Version 6.156.0 revalidates every signed CMS publication against the current
target-locale quality policy and approval lifetime immediately before leasing
and again after the outer operation guard, directly before publisher access.
Policy drift during an active lease therefore cannot reach the network.

Authenticated deliveries whose release evidence is stale or whose approvals
have expired are quarantined terminally with content-free reason codes, without
consuming an attempt. A bounded transaction skips up to 24 such entries so one
obsolete website version cannot block a valid following delivery. Structural,
payload, and signature failures remain hard integrity errors; structural-only
tombstone validation can still remove obsolete published content safely.
...
The portable fail-closed hook is
[`pre_output_guard.py`](integrations/pre_output_guard.py). It accepts
`task_kind`, target, locale, receipt, and the complete source for translations
as JSON on stdin and exits nonzero when verification fails. For `rewrite`, the
trusted caller also supplies the exact profile, request, session, creator, and
channel binding; the hook requires the configured isolated service, authorizes
the exact result, and consumes its one-time delivery grant before returning
`allow: true`. It never falls back to a same-user signing key for rewrites and
never returns source text, target text, receipts, or grants in its decision.
For translations it only reads an existing owner-only verification key and
never creates or repairs signing state. Host-specific adapters must pass the
candidate output into this contract; a host hook that exposes no candidate text
cannot enforce output validation.
