...
The guard also measures linguistic volume and segment coverage and blocks substantial targets that are still identical to the complete source. Treat a volume, segment-count, or long source-identity failure as blocking. The mandatory MCP `release_translation` path repeats the unconditional identity and auto-detected volume checks; caller attestations cannot suppress them. Never describe exit code `0` as proof that a translation is complete, faithful, or native: it proves only the deterministic structure, protected-token, identity, volume, and Unicode checks that actually ran.
