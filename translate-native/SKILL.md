...
For Claude Code, use the Version 6.3 authenticated loopback Streamable HTTP MCP together with the Version 6.4 `translate-native` plugin. The service survives client disconnects; the plugin injects mandatory session policy and requires `Stop` and `SubagentStop` to consume a fresh isolated verification record bound to the exact final response. A successful tool call alone is insufficient. Do not edit the text after release, reuse a prior record, or fall back to an unguarded response when the HTTP MCP or isolated guard is unavailable.
