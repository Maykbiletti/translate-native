...
## Naturally revise original writing

For same-language editing, proofreading, or making an AI draft read naturally,
use the host's `rewrite_text` operation when configured. Supply the complete
original as `source_text`, the exact language, an explicit host-registered
`profile_id`, a stable `request_id`, and the content type. Keep that request ID
on an unchanged retry; changed input needs a new ID. Do not supply a candidate,
review attestations, or model-controlled reviewer configuration.

This is not translation and not authorship detection. Preserve facts, numbers,
negation, modality, personal voice, intentional repetition, terminology and
protected syntax. Apply a regional variety or dialect only when explicitly
requested through the selected profile. Do not impose German or English style
norms on another language. Missing reliable variety evidence requires independent
review, not invented dialect. A clean orthography report or `NO_SIGNALS` is not
native-quality acceptance.

The worker creates the revision, delegates a source-blind native review, then
delegates a separate original-versus-revision preservation review. The existing
trusted host must first inject a short-lived, one-time rewrite context bound to
the exact original bytes, locale, content type, profile policy, request ID,
current session epoch and writer identity. Claude hosts replace model-supplied
original, request and content fields with those trusted bindings. Never invent, reuse or copy that
context. The Guard alone signs a `rewrite`-purpose receipt. Any edit requires a fresh run.
Keep blocked drafts and internal reviews internal. Missing host configuration
must block explicitly; never silently send rewriting through `release_response`
...
The receipt remains bound to its request, session, epoch, writer and Guard boot
and may authorize only one delivery grant. An exact authorization retry returns
that same grant; a changed retry blocks.

## Mandatory host enforcement

When a host exposes the BLUN Language Gateway, every user-visible natural-language answer must pass through it before it can reach the user, a file, a commit, or a deployment. The host must set `task_kind: response` for an agent's own answer, `task_kind: rewrite` for same-language revision of an original, and `task_kind: translation` for every translation or localization output. Never bypass the gateway because the task is short, conversational, generated by another agent, or already appears fluent. A gateway `BLOCK`, missing receipt, stale receipt, edited target, wrong receipt purpose, or unavailable mandatory gateway blocks release.

For an ordinary answer, call `release_response` with the complete final text and exact host-supplied language tag. The trusted host must inject a one-time review context and delegate a fresh source-blind native-language review; the writing agent cannot approve itself or supply review evidence. For a translation, load this skill before drafting and call only `release_translation` with the complete source-target pair and all seven attestations. Never downgrade a translation to the response path or omit its source. The host—not the agent—must own task classification and expected language.
