...
For Claude Code, use the Version 6.3 authenticated loopback Streamable HTTP MCP together with the Version 6.7.1 `translate-native` plugin. The service survives client disconnects; the plugin injects mandatory session policy and requires `Stop` and `SubagentStop` to consume a fresh service-signed one-time delivery grant bound to the exact final response, Claude session, agent identity, guard version, and current service boot. Keep the operating-system health monitor enabled so it probes the isolated signer and complete MCP tool path every minute, repairs the failed dependency once, applies persistent exponential backoff after repeated repair failures, and remains fail-closed during recovery. The updater must synchronize an already-installed Claude plugin cache to the exact runtime version and report a degraded state on mismatch; a downloaded cache update becomes active only after `/reload-plugins` or a new session. A successful tool call or locally forged hook record is insufficient. Do not edit the text after release, reuse a prior grant, or fall back to an unguarded response when the HTTP MCP or isolated guard is unavailable.
