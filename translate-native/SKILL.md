...
The guard also measures linguistic volume and segment coverage and blocks substantial targets that are still identical to the complete source. For structured formats, it additionally compares human-language segments and blocks an untranslated value even when the rest of the file changed or values were reordered. JSON alignment is independent of property order; other formats check identity across the complete target segment set. Short product names and complete short copyright/rights notices may remain fixed, but prose that merely mentions copyright is not exempt. Parse a copyright exemption as a complete marker, year, and conservative owner name; never treat short title-cased prose as an owner automatically. Treat a volume, segment-count, long source-identity, or `unchanged-linguistic-segment` failure as blocking. The mandatory MCP `release_translation` path repeats the unconditional identity, structured-segment identity, and auto-detected volume checks; caller attestations cannot suppress them. Never describe exit code `0` as proof that a translation is complete, faithful, or native: it proves only the deterministic structure, protected-token, identity, volume, and Unicode checks that actually ran.
