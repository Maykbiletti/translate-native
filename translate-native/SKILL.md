...
For Claude Code, prefer the Version 6.3 authenticated loopback Streamable HTTP MCP installed at user scope. It is a persistent, stateless service with an operating-system restart policy, so a broken client connection does not terminate the MCP runtime and the next request can reconnect. The dynamic header helper reloads its access token on every connection. Do not fall back to an unguarded response when the HTTP MCP or isolated guard health check fails; mandatory delivery still blocks outside the agent.
